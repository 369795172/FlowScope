...
The system SHALL provide an API endpoint to query all upstream paths (sources) leading to a given dataset, with optional business usage indicators.
...
- **AND** paths include business usage indicators (which datasets in path have business usage)
...
- **WHEN** API receives request with business usage filter
- **THEN** only paths that include datasets with business usage are returned
- **AND** paths are marked as business-critical if they include business usage

### Requirement: Downstream Path Query
The system SHALL provide an API endpoint to query all downstream paths (destinations) from a given dataset, with optional business usage indicators.
...
- **THEN** paths are pruned early based on filters (layer, system, business usage)
...
- **AND** paths include business usage indicators

### Requirement: Business-Critical Path Identification
The system SHALL identify and highlight paths that include business usage as business-critical.
...
- **AND** path responses include business usage indicators for each dataset in the path
- **AND** business-critical paths are highlighted in API responses
